...
export default function Employee() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editingEmployee) {
        // Update locally
        setEmployees((prev) =>
          prev.map((emp) => (emp.id === editingEmployee.id ? res.data.employee : emp))
        );
      } else {
        // Add locally
  ...
  const handleDelete = async (emp) => {
    ...
    try {
      await axios.delete(`${BASE_URL}/deleteEmployee/${emp.id}`);
  ...
  return (
    <div className="relative p-4 max-w-full overflow-x-auto">
        ...
        <table className="min-w-full text-sm sm:text-base">
          <thead className="bg-gradient-to-b from-[#1E3A8A] to-[#4891e4] text-white">
            <tr className="border-b border-blue-300">
              <th className="text-left text-sm px-2 py-2">First Name</th>
              <th className="text-left text-sm px-2 py-2">Middle Name</th>
              <th className="text-left text-sm px-2 py-2">Last Name</th>
              <th className="text-left text-sm px-2 py-2">Email</th>
              <th className="text-left text-sm px-2 py-2">Position</th>
              <th className="text-left text-sm px-2 py-2">Department</th>
              <th className="text-left text-sm px-2 py-2">Salary</th>
      ...
      {modalOpen && (
        <div
          className="fixed top-17 right-0 bottom-0 z-50 w-full sm:w-[90%] md:w-[500px] bg-white shadow-lg overflow-y-auto transition-transform transform translate-x-0"
